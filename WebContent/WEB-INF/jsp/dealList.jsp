			...
			<div class="nTab">
				<div class="TabTitle">
					<ul id="myTab1">
						...
						<li id="li3" class="active" onclick="window.location.href='/FLOP/order/listToDeal.do'">预约处理</li>
					</ul>
				</div>
				<div class="statics">
						...
						<div id="newsList">
							<table id="newsTable">
								...
								<tr>
									...
									<td>${order.appoint.date}</td>
									<td>${order.appoint.lesson}</td>
									<td>
										<c:choose>
											<c:when test="${order.status == 'verify'}">
												<a href="accept.do?orderId=${order.id}">接受</a>|<a href="reject.do?orderId=${order.id}">拒绝</a>
											</c:when>
											<c:when test="${order.status == 'accept'}">
												<div id="status">已接受</div>
											</c:when>
