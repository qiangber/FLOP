	...
	<div id="wrap">
		<div id="left">
			<div class="funcbox">
				<ul>
					...
					<li class="now"><a href="${pageContext.request.contextPath}/user/list.do?type=teacher">老师用户管理</a></li>
		...
		<div id="right">
			...
			<div class="nTab">
				<div class="TabTitle">
					<ul id="myTab1">
						<li id="li1" class="normal" onclick="window.location.href='list.do?type=teacher'">用户列表</li>
					...
					<div class="phone_pic" id="newsList0">
						<form action="save.do" method="POST" name="formUser" onsubmit="return onsubmitCheck();">
					        <div class="pass">
					            ...
					            <div class="item"><div class="title">账号：</div><input name="username" id="username" type="text" value="${user.username}" autocomplete="off"/><input type="hidden"></div>
					            ...
					            <div class="item">
					            	...
					            	<label><input name="writing" type="checkbox" value="true" checked="checked" />写作老师</label>					        
					            	</c:when>
					            	<c:otherwise>
					            	<label><input name="writing" type="checkbox" value="true"/>写作老师</label>
					            	</c:otherwise>
					            	</c:choose>
					            	<c:choose>
					            	<c:when test="${user.userInfo.speaking == true}">
					            	<label><input name="speaking" type="checkbox" value="true" checked="checked" />口语老师</label>					        
					            	</c:when>
					            	<c:otherwise>
					            	<label><input name="speaking" type="checkbox" value="true"/>口语老师</label>
					            	</c:otherwise>
					            	</c:choose>
					            	<c:choose>
					            	<c:when test="${user.userInfo.lab == true}">
					            	<label><input name="lab" type="checkbox" value="true" checked="checked" />实验室预约</label>					        
					            	</c:when>
					            	<c:otherwise>
					            	<label><input name="lab" type="checkbox" value="true"/>实验室预约</label>
					            	</c:otherwise>
					            	</c:choose>					            	
					            </div>
